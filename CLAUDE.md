...
- ドメインエンティティ(`Post`/`Diary`など)の不変条件(空文字禁止・長さ制限など)は、ユーザー入力由来のフィールドだけでなく、AIが生成する結果が入るフィールド(`Diary.body`など)にも同様にBean Validationを付与する。「ユーザー入力ではないから」という理由で検証を省略しない。AI呼び出しが空文字列や想定外の結果を返した場合に、それを検知できるのはドメイン層の制約だからである。
- フォーム(`form/`配下)のBean Validationだけに頼らず、対応するドメインエンティティ側にも同じ制約(`@NotBlank`/`@Size`など)を付与する。フォームは特定の入力経路(HTTPリクエスト)を検証しているに過ぎず、将来バッチ処理やAPIなど別の経路からドメインオブジェクトが組み立てられる場合、フォーム層は素通りされる。ドメイン層自身の制約だけが、どの経路でも共通して効く最後の砦になる。
- ユーザー入力をAIへのプロンプトに埋め込む処理(分類・生成など)を書く場合、無害化(サニタイズ)や区切りタグでの囲い込みは、呼び出す側のservice実装ごとに個別に行わない。対象のドメインオブジェクト自身にメソッドとして持たせる(例: `Post.bodyForPrompt()`)。これにより、どのAI連携クラス・どの呼び出し経路を通っても無害化が漏れなく効く。無害化はプロンプトへ渡す文字列にのみ適用し、永続化される本文自体は変更しない。
...
- ログにパスワード・認証トークン・Anthropic APIキーなど機微情報を出力しない。SQLのバインドパラメータ実値を出力するログ設定(`logging.level.org.hibernate.orm.jdbc.bind: trace`など)は、素の`application.yaml`に書かず、`spring.config.activate.on-profile`で明示的にスコープした`application-dev.yaml`のようなdev限定ファイルに閉じ込める。素の`application.yaml`に書くと、service層で意図的にマスクしている本文などのログ出力方針が、Hibernateのログ経由で無意味になる。
...
- フォーム入力にBean Validationが付与されているか(クライアント側検証のみに頼っていないか)。ドメインエンティティ側にも同じ制約が付与されているか(フォームをバイパスする経路でも効くか)。AI生成結果が入るフィールドにも同様の制約があるか。
- AIへのプロンプトに埋め込むユーザー入力が、区切りタグでの囲い込み・無害化(サニタイズ)をされているか。無害化がservice層の呼び出し箇所ごとの実装依存になっておらず、対象のドメインオブジェクト(`Post.bodyForPrompt()`など)に委譲されているか。
- SQLバインド値などの機微情報を出力するログ設定が、devプロファイル限定になっているか(素の`application.yaml`に書かれていないか)。
