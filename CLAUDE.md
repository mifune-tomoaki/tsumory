...
想定しているエンティティは以下の3つ(いずれも`domain/`配下に置く)。

- **User**: ログイン認証を持つユーザー。認証情報(メールアドレス/ユーザー名、パスワードハッシュなど)を保持し、Spring Securityと連携する。
- **Post(つぶやき)**: ユーザーが投稿する短文。本文は100文字程度を上限とし、投稿者(User)・本文・投稿日時を保持する。
- **Diary(日記)**: その日のPostをAIがまとめて生成する日記。**ユーザーごとに1日1本**(User × 日付の組み合わせで一意)。対象ユーザー・対象日付・本文・生成日時を保持する。

関係性・制約:

- User 1 : N Post
- User 1 : N Diary。ただし同一ユーザー・同一日付のDiaryは1件のみ(`user_id` + `diary_date`にユニーク制約。Flywayマイグレーションで表現する)。
- DiaryはPost側からの直接の外部キーを持たず、生成時に対象ユーザー・対象日付のPostを検索して集約する想定。
- Postの本文の長さ制限はBean Validation(`@Size`など)でform/domain双方に適用する。
