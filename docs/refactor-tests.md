...
> **✅ 対応済み**: [code-review-resolutions.md](./code-review-resolutions.md#refactor-testsmd-指摘2〜9-domainテスト群の残り指摘)を参照。
...
| 2 | `DiaryTest`に同種のファクトリヘルパーが無い | ✅対応済み |
| 3 | `Instant.now()`/`LocalDate.now()`が固定クロック規約から外れている | ✅対応済み |
| 4 | `DiaryTest`が既存のサンプル本文定数を使っていない | ✅対応済み |
| 5 | `DiaryTest`のテストメソッド名だけ命名規約から外れている | ✅対応済み |
| 6 | `TestFixtures`のクラスコメントが実態と不一致 | ✅対応済み |
| 7 | `Post.body`のBean Validation制約に専用テストが無い | ✅対応済み |
| 8 | Bean Validationテストのセットアップ重複(7採用時) | ✅対応済み |
| 9 | `PromptSanitizerTest`と`PostTest`のテスト内容重複 | ✅対応済み(コメントで意図を明記) |
