...
「今は対応しないが、将来ある条件を満たしたら再検討する」と決めた設計判断の記録。経緯と、議論を再開すべき条件をまとめたもの。

## [refactor-tests.md](./refactor-tests.md)

ドメインテスト群(`PostTest`/`DiaryTest`/`PromptSanitizerTest`)を対象にした、重複・一貫性・カバレッジの観点のリファクタリング指摘。
