...
class AnthropicPostCategorizerTest {
  ...
  void buildUserMessage_wrapsBodyInDelimiterTags() {
    String message = categorizer.buildUserMessage(TestFixtures.post(POST_BODY_MORNING));
  ...
  void buildUserMessage_neutralizesAttemptToForgeTheClosingDelimiter() {
    ...
    String message = categorizer.buildUserMessage(TestFixtures.post(maliciousBody));
